...
device.shell("input keyevent 3") #Home (To turn the phone on and access the lock screen)
device.shell("input keyevent 145 keyevent 146 keyevent 147 keyevent 148 && input keyevent 160") # PIN: 1 2 3 4 Enter 
...
sleep(0.5)
device.shell('input text "gabriel%stressoldi%simages" && input keyevent 66') # "%s" to use space
sleep(5)
device.shell("input keyevent 27") #Camera
sleep(2)
device.shell("input keyevent 3") #Home

#To find adb key events, just search "adb key events" on google
